...
DATA_DESTINATION = "postgres/data"
SQL_DESTINATION = "postgres/sql"
TRAIN_SQL_FILE = "02_" + TRAIN_TABLE + ".sql"
TEST_SQL_FILE = "03_" + TEST_TABLE + ".sql"
...
if len(sys.argv) > 1 and sys.argv[1] == "--download":
    ...
    download_mnist_files(DATA_DESTINATION)


print("Creating SQL file with training data")
prepare_sql(
    os.path.join(DATA_DESTINATION, MNIST_TRAIN_LABEL_FILE),
    os.path.join(DATA_DESTINATION, MNIST_TRAIN_IMAGE_FILE),
    os.path.join(SQL_DESTINATION, TRAIN_SQL_FILE),
    TRAIN_TABLE
)

print("Creating SQL file with test data")
prepare_sql(
    os.path.join(DATA_DESTINATION, MNIST_TEST_LABEL_FILE),
    os.path.join(DATA_DESTINATION, MNIST_TEST_IMAGE_FILE),
    os.path.join(SQL_DESTINATION, TEST_SQL_FILE),
    TEST_TABLE
)
